  ...
  return (
    ...
    >
        ...
        <div className="kwr-slide" key={slide}>
          {slide === 0 && (
            <div className="kwr-intro">
              <span className="aria-orb kwr-aria" aria-hidden />
              <span className="kwr-kicker">This week in review</span>
              <h1 className="kwr-h1 serif">Your week<br />with Aria</h1>
              <div className="kwr-rankchip">
          ...
          {slide === 1 && (
            <div className="kwr-stats">
              <span className="kwr-kicker">By the numbers</span>
              <h2 className="kwr-h2 serif">You showed up.</h2>
              <div className="kwr-statgrid">
                <div className="kwr-stat">
                  <span className="kwr-statnum"><CountUp to={data.daysClosed} active={slide === 1} /></span>
                  <span className="kwr-statlabel">Days closed</span>
                </div>
                <div className="kwr-stat">
                  <span className="kwr-statnum"><CountUp to={data.promisesKept} active={slide === 1} /></span>
                  <span className="kwr-statlabel">Promises kept</span>
                </div>
                <div className="kwr-stat">
                  <span className="kwr-statnum"><CountUp to={data.reflections} active={slide === 1} /></span>
          ...
          {slide === 2 && (
            <div className="kwr-momentum">
              <span className="kwr-kicker">Momentum</span>
              <div className="kwr-xp">
          ...
          {slide === 3 && (
            <div className="kwr-closing">
              <span className="aria-orb kwr-aria" aria-hidden />
              <p className="kwr-said serif">{closingLine}</p>
              <div className="kwr-rankchip kwr-rankchip--soft">
                <span className="kwr-rankchip-rank">{data.rank}</span>
              </div>
              <div className="kwr-actions">
                <button className="btn btn-primary kwr-btn" onClick={share}>
                ...
                <button className="btn btn-ghost kwr-btn kwr-btn--done" onClick={finish}>Done</button>
              </div>
            </div>
          )}
        </div>
      ...
      <style>{`
        ...
        .kwr-seg { flex: 1; height: 3px; border-radius: 999px; background: rgba(255,245,236,.26); overflow: hidden; }
        .kwr-seg-fill { display: block; height: 100%; width: 0; border-radius: 999px; background: #fff5ec; box-shadow: 0 0 8px rgba(255,236,214,.5); }
        ...
        .kwr-slide { width: 100%; max-width: 560px; text-align: center; animation: kwrRise .55s var(--ease) both; }
        ...
        .kwr-h1 { margin: 0; font-weight: 600; font-size: clamp(2.9rem, 12vw, 4.6rem); line-height: 1.02; letter-spacing: -.02em; color: #fff; text-shadow: 0 6px 30px rgba(0,0,0,.35); }
        .kwr-h2 { margin: 0 0 1.5rem; font-weight: 600; font-size: clamp(2.1rem, 8vw, 3rem); color: #fff; }

        .kwr-rankchip { display: inline-flex; align-items: center; gap: .55rem; margin-top: 1.3rem; padding: .5rem 1.05rem; border-radius: 999px;
          background: rgba(255,245,236,.14); border: 1px solid rgba(255,245,236,.28); backdrop-filter: blur(6px);
        ...
        .kwr-stat { display: flex; flex-direction: column; align-items: center; gap: .3rem; padding: 1rem .4rem;
          border-radius: var(--r-lg); background: rgba(255,245,236,.10); border: 1px solid rgba(255,245,236,.16); backdrop-filter: blur(4px); }
        .kwr-statnum { font-family: var(--font-display); font-weight: 700; font-size: clamp(2.8rem, 13vw, 4.2rem); line-height: 1; color: #fff; font-variant-numeric: tabular-nums; text-shadow: 0 4px 22px rgba(0,0,0,.3); }
        ...
        .kwr-streak { display: inline-flex; align-items: center; gap: .6rem; padding: .7rem 1.2rem; border-radius: 999px;
          background: rgba(255,245,236,.12); border: 1px solid rgba(255,245,236,.24); backdrop-filter: blur(6px); }
        ...
        .kwr-btn { pointer-events: auto; font-size: 1.05rem; padding: .85rem 1.7rem; }
        .kwr-btn--done { background: rgba(255,245,236,.12); border-color: rgba(255,245,236,.32); color: #fff5ec; }
        .kwr-btn--done:hover { background: rgba(255,245,236,.2); border-color: rgba(255,245,236,.5); }

        @keyframes kwrFade { from { opacity: 0; } to { opacity: 1; } }
        @keyframes kwrRise { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: none; } }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .kwr-bg { transition: none !important; }
